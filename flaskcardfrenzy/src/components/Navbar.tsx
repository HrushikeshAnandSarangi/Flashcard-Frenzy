...
const Button: FC<{
  ...
  const baseStyle = "inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-400 focus:ring-offset-slate-900 disabled:opacity-50 disabled:pointer-events-none";
...
// The main Navbar component with the neon theme
const FuturisticNavbar: FC = () => {
  ...
  const signedOutNav = [
    { text: 'Sign Up', href: '/signup', icon: <UserPlus className="h-5 w-5" /> },
  ];
  ...
  const NavLink: FC<{ href: string, children: ReactNode, onClick?: () => void }> = ({ href, children, onClick }) => (
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-4 space-y-2 border-t border-cyan-500/30">
              {navItems.map((item) => (
                <NavLink key={item.text} href={item.href} onClick={handleMenuClose}>
                  {item.icon}
              ...
              <div className="pt-3 px-2">
                {isSignedIn ? (
                  <Button onClick={handleAuthToggle} className="w-full justify-center">
                    <LogOut className="h-4 w-4 mr-2" />
                    Log Out
                  </Button>
                ) : (
